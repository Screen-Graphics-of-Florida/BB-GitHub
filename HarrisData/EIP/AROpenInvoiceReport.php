...
$page_title            = "A/R Open Invoice Report";
$scriptName            = "AROpenInvoiceReport.php";
$scriptVarBase         = "{$genericVarBase}&amp;backHome=" . urlencode(trim($backHome)) . "&amp;reportSelType=" . urlencode(trim($reportSelType));
$baseURL               = "{$homeURL}{$phpPath}{$scriptName}{$scriptVarBase}";
$submitNoSelection     = "";
$submitCallProgram     = "CAROIL";
$submitEnvProgram      = "HAROIL";
$submitEnvPrinter      = "HAROILPF";
...
if ($tag == "REPORT") {
	...
	print "\n   if (editdate(document.Chg.FullyPaidDate) ";
	...
	print "\n    && editFromToOper(document.Chg.frOrderNumber,document.Chg.toOrderNumber,document.Chg.operOrderNumber, 8) ";
	print "\n    && editdate(document.Chg.frOrderDate) ";
	print "\n    && editdate(document.Chg.toOrderDate) ";
	print "\n    && editFromToOper(document.Chg.frOrderDate, document.Chg.toOrderDate,  document.Chg.operOrderDate, 'D') ";
	...
	if ($errFound != "" || $scheduleJobSwitch == "Y") {
		...
		if ($errFound != "") {
			...
			$Err_PDTL=DecatErr_Field("@@pdtl", "PaymentDetail");
			$Err_FULL=DecatErr_Field("@@full", "PaidInFull");
			$Err_PDDT=DecatErr_Field("@@pddt", "FullyPaidDate");
		...
		$PDTL=Decat_Field("@@pdtl", $edtVar);
		$FULL=Decat_Field("@@full", $edtVar);
		$PDDT=Decat_Field("@@pddt", $edtVar);
	...
	print "\n     <table $contentTable> ";
	Build_Fld_Entry("Print Payment Detail","PaymentDetail","inputalph","YORN","PDTL",$PDTL,$Err_PDTL,"1","1","","","");
	// Fully Paid
	$textOvr=SetTextOvr($Err_FULL);
	print "\n         <tr><td class=\"dsphdr\"><span $textOvr>Include Fully Paid Invoices</span></td>";
	Build_Fld_Entry("Include Fully Paid Invoices","PaidInFull","inputalph","YORN","FULL",$FULL,$Err_FULL,"1","1","","","Y");
	$textOvr=SetTextOvr($Err_PDDT);
	print "\n             <td class=\"dsphdr\"><span $textOvr>&nbsp; After</span></td>";
	Build_Fld_Entry("After","FullyPaidDate","inputnmbr","Date","",$PDDT,$Err_PDDT,"1","1","","","Y");
	print "\n         </tr> ";
	DspErrMsg($Err_FULL);
	DspErrMsg($Err_PDDT);
	print "\n     </table> ";
	...
	Build_AdvSrch_Entry("Distribution Period","frDistPeriod","toDistPeriod","operDistPeriod","opersel_num2_short","DP","4","4");
	Build_AdvSrch_Entry("Order Number","frOrderNumber","toOrderNumber","operOrderNumber","OperSel_num2_Short","N","8","8");
	Build_AdvSrch_Entry("Order Date","frOrderDate","toOrderDate","operOrderDate","opersel_num2_short","D","6","6");
...
if ($tag == "WILDCARD" || $tag == "Edit_Data") {
	...
	$returnValue=Range_WildCard("IVPER", "Distribution Period", $_POST['frDistPeriod'], $_POST['toDistPeriod'], "", $_POST['operDistPeriod'], "DP");
	$returnValue=Range_WildCard("IVORD", "Order Number", $_POST['frOrderNumber'], $_POST['toOrderNumber'], "", $_POST['operOrderNumber'], "N");
	$returnValue=Range_WildCard("IVORDT", "Order Date", $_POST['frOrderDate'], $_POST['toOrderDate'], "", $_POST['operOrderDate'], "D");
	...
	if (!isset($_POST['PaymentDetail'])) {$_POST['PaymentDetail']="N";}  Concat_Field("@@pdtl", $_POST['PaymentDetail']);
	if (!isset($_POST['PaidInFull'])) {$_POST['PaidInFull']="N";}  Concat_Field("@@full", $_POST['PaidInFull']);
	Concat_Field("@@pddt", $_POST['FullyPaidDate']);
	...
	$returnValue=Selection_Edit_Handle("HARSOL_W", $profileHandle, $dataBaseID, $submitSchedule, $errFound, $edtVar, $errVar, $wrnVar);
