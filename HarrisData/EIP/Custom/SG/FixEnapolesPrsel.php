<?php
// FixEnapolesPrsel.php
// Fix: SYPORR rows for native portals have PRSEL='' instead of 'Y'.
// Menu.php filters AND PRSEL='Y' so they never show.
// Fix: UPDATE SYPORR SET PRSEL='Y' WHERE PRROLE='ENAPOLES' AND PRSEL=''.
//
// Preview: https://portal.screen-graphics.com:5601/Custom/SG/FixEnapolesPrsel.php
// Execute: https://portal.screen-graphics.com:5601/Custom/SG/FixEnapolesPrsel.php?confirm=FIX
...
if (isset($_GET['confirm']) && $_GET['confirm'] === 'FIX' && $toFix > 0) {
    ...
    // Backup ALL SYPORR rows for ENAPOLES
    $lines = array(
        '-- FixEnapolesPrsel backup: ALL SYPORR rows for ENAPOLES',
    ...
    $backupFile   = $outDir . '/FixEnapolesPrsel_' . $ts . '.sql';
...
<head>
<meta charset="utf-8">
<title>Fix ENAPOLES PRSEL</title>
<style>
...
<div class="hdr">Fix ENAPOLES: Set PRSEL='Y' for Native Portals</div>
