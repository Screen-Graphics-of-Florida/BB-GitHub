...
$scriptName    = "ApplCashPaymentCash.php";
...
$InvBalSQL     = "IVIVAM-IVNPOS-IVPPOS-(-Coalesce(PEAMT-(-PEDAMT),0))-(-Coalesce(OFF_PEAMT,0))";
...
array("", "Discount", $viewCheckBoxURL, "6", "1"));
...
if ($tag == "MASTERSEARCH"){
	...
	require_once 'ApplCashPaymentInvoiceIncludeMasterSearch.php';
...
if ($tag == "ORDERBY"){
	if     ($sequence == "Select")        {$orby = array(array("PESSEQ" ,"A","Selected Sequence"));}
	elseif ($sequence == "Invoice")       {$orby = array(array("IVAINV" ,"A","Invoice"),array("IVISEQ","A",""),array("IVISEQ","A",""));}
	...
	elseif ($sequence == "PayOffSubCode") {$orby = array(array("PEPOSB" ,"A","Pay Off Payment Code"),array("IVAINV" ,"A","Invoice"),array("IVISEQ","A",""));}
	elseif ($sequence == "PayOffAmount")  {$orby = array(array("PEPOAM" ,"A","Pay Off Amount"),array("IVAINV" ,"A","Invoice"));}
...
$stmtSQL .= "       ,Coalesce(PEPOSB, ' ')   as PEPOSB ";
$stmtSQL .= "       ,Coalesce(PEPOTP,' ') as PEPOTP ";
$stmtSQL .= "       ,Coalesce(PEPOEN,0)   as PEPOEN ";
$stmtSQL .= "       ,Coalesce(OFF_PEAMT,0)   as PEPOAM ";
...
$fileSQL .= " left join Table (Select PEISEQ as OFF_PEISEQ,PEPTYP as OFF_PEPTYP,PEENID as OFF_PEENID,Sum(PEAMT) as OFF_PEAMT From ARPYEN Where (PEBCHN,PEBCHD,PEBCHB,PETYPE,PEID,trim(PECHK),PEPMID)=($fromBatchNumber,$fromBatchDate,$fromBatchBank,'$fromType',$fromID,'" . trim($fromDocument) . "','$paymentID') Group By PEISEQ,PEPTYP,PEENID) as PAYOFF on (OFF_PEISEQ,OFF_PEPTYP,OFF_PEENID)=(a.IVISEQ,b.PEPOTP,b.PEPOEN) ";
...
if ($CRPARF=="Y") {$selectSQL .= " and Coalesce(LOFACT,' ')<>'Y' ";}  // Cannot be Factored Location
...
require_once 'ApplCashPaymentCashJava.php';
...
if ($formatToPrint != "Y"){
	...
	if ($columnDisplay['PEPOAM']=="Y") {
		$returnValue=OrderBy_Sort("PEPOSB"); $sortVar=$returnValue['sortedBy']; $sortPoint=$returnValue['sortPoint'];
		print "\n <th class=\"colhdr$sortVar\"><a href=\"{$baseURL}&amp;tag=ORDERBY&amp;sequence=PayOffSubCode\"   title=\"Sequence By Pay Off Payment Code\">{$sortPoint}Pay Off Payment Code</a></th>";
		$returnValue=OrderBy_Sort("PEPOAM"); $sortVar=$returnValue['sortedBy']; $sortPoint=$returnValue['sortPoint'];
		print "\n <th class=\"colhdr$sortVar\"><a href=\"{$baseURL}&amp;tag=ORDERBY&amp;sequence=PayOffAmount\"    title=\"Sequence By Pay Off Amount\">{$sortPoint}Pay Off Amount</a></th>";
	}
	...
	if ($columnDisplay['PEPOAM']=="Y" && ($harced_OPT['sec_03']=="Y" || $harced_OPT['sec_06']=="Y" && $CRDPYC!="")) {
		print "\n     <td class=\"entry\" nowrap><input type=\"text\" name=\"addPayOffSubCode\" id=\"addPayOffSubCode\" size=\"4\" maxlength=\"4\"> ";
		print "\n                         <a href=\"{$homeURL}{$phpPath}ARPmtSubCodeSearch.php{$scriptVarBase}&amp;docName=Chg&amp;fldName=addPayOffSubCode&amp;fldDesc=none&amp;specificBatchType={$BMBCHT}&amp;specificPmtType=PAYOFF\" onclick=\"$searchWinVar\">$searchImage</a></td> ";
	}
	...
	if ($columnDisplay['PEPOAM']!="Y" || $harced_OPT['sec_03']=="N" && ($harced_OPT['sec_06']=="N" || $CRDPYC=="")) {print "\n     <td><input type=\"hidden\" name=\"addPayOffSubCode\" id=\"addPayOffSubCode\"></td> ";}
	...
	while ($row = db2_fetch_assoc($sqlResult, $startRow)){
		...
		if ($row['ARPYENERROR']>0) {
			...
			$PEPOSB_ERROR=RetValue("(PRBCHN,PRBCHD,PRBCHB,PRTYPE,PRID,trim(PRCHK),PRPTYP,PRPMID,PRISEQ,PRENID,PRCOLM)=($fromBatchNumber,$fromBatchDate,$fromBatchBank,'$fromType',$fromID,'" . trim($fromDocument) . "','$paymentType','$paymentID',$row[PEISEQ],$row[PEENID],'PEPOSB') ", "ARPYER inner join HDERROR on ERER#=PRERR", "trim(Coalesce(ERERDS,PRERR,' '))");
		} else {
			$PEAMT_ERROR="";
			$PEDAMT_ERROR="";
			$PEPOSB_ERROR="";
		...
		if ($row['PEAMT']!=0) {print "\n     <td class=\"inputnmbr\" nowrap><input type=\"text\"     name=\"amt{$row['PEISEQ']}_{$row['PEENID']}\" id=\"amt{$row['PEISEQ']}_{$row['PEENID']}\" value=\"" . number_format($row['PEAMT'],2, '.', '') . "\" size=\"8\" maxlength=\"15\" $FldStyle onChange=\"editPEAMTCash('$row[PEISEQ]','$row[PEENID]','$row[MINPEENID]','$sdscDefault')\"> ";}
		else                  {print "\n     <td class=\"inputnmbr\" nowrap><input type=\"text\"     name=\"amt{$row['PEISEQ']}_{$row['PEENID']}\" id=\"amt{$row['PEISEQ']}_{$row['PEENID']}\" value=\"\" size=\"8\" maxlength=\"15\" $FldStyle onChange=\"editPEAMTCash('$row[PEISEQ]','$row[PEENID]','$row[MINPEENID]','$sdscDefault')\"> ";}
		print "\n                                    <input type=\"checkbox\" name=\"sclc{$row['PEISEQ']}_{$row['PEENID']}\" id=\"sclc{$row['PEISEQ']}_{$row['PEENID']}\" value='S' $row[CHECKCALCULATE] onClick=\"editPESCLCCash('$row[PEISEQ]','$row[PEENID]','$row[MINPEENID]','$sdscDefault')\" title=\"Calculate Payment\"></td> ";
		...
		if ($columnDisplay['PEDAMT']=="Y") {
			...
			if ($row['PEDAMT']!=0) {print "\n <td class=\"$dscClass\" nowrap><input type=\"text\"     name=\"damt{$row['PEISEQ']}_{$row['PEENID']}\" id=\"damt{$row['PEISEQ']}_{$row['PEENID']}\" value=\"" . number_format($row['PEDAMT'],2, '.', '') . "\" size=\"8\" maxlength=\"15\" $FldStyle onChange=\"editPEDAMTCash('$row[PEISEQ]','$row[PEENID]','$row[MINPEENID]','$sclcDefault') \">  ";}
			else                   {print "\n <td class=\"$dscClass\" nowrap><input type=\"text\"     name=\"damt{$row['PEISEQ']}_{$row['PEENID']}\" id=\"damt{$row['PEISEQ']}_{$row['PEENID']}\" value=\"\" size=\"8\" maxlength=\"15\" $FldStyle onChange=\"editPEDAMTCash('$row[PEISEQ]','$row[PEENID]','$row[MINPEENID]','$sclcDefault') \">  ";}
			print "\n                                <input type=\"checkbox\" name=\"sdsc{$row['PEISEQ']}_{$row['PEENID']}\" id=\"sdsc{$row['PEISEQ']}_{$row['PEENID']}\" value=\"S\" $row[CHECKDISCOUNT] onClick=\"editPESDSCCash('$row[PEISEQ]','$row[PEENID]','$row[MINPEENID]','$sclcDefault')\" title=\"Select Discount\"></td>  ";
		}

		// Entry - Pay Off Payment Code, Amount
		if ($columnDisplay['PEPOAM']=="Y") {
			if ($PEPOSB_ERROR!="") {$FldStyle="style=\"background-color: $errorBackground\" title=\"$PEPOSB_ERROR\"  ";}
			else                   {$FldStyle="";}
			print "\n <td class=\"inputalph\"> ";
			if ($harced_OPT['sec_03']=="Y" || $harced_OPT['sec_06']=="Y" && $CRDPYC!="") {
				print "\n <input type=\"text\" name=\"posb{$row['PEISEQ']}_{$row['PEENID']}\" id=\"posb{$row['PEISEQ']}_{$row['PEENID']}\" value=\"" . rtrim($row['PEPOSB']) . "\" size=\"4\" maxlength=\"4\" $FldStyle onBlur=\"editPEPOSBCash('$row[PEISEQ]','$row[PEENID]','$row[MINPEENID]','$sclcDefault','$sdscDefault')\"> ";
				print "\n <a href=\"{$homeURL}{$phpPath}ARPmtSubCodeSearch.php{$scriptVarBase}&amp;docName=Chg&amp;fldName=posb{$row['PEISEQ']}_{$row['PEENID']}&amp;fldDesc=none&amp;specificBatchType={$BMBCHT}&amp;specificPmtType=PAYOFF\" onclick=\"$searchWinVar\">$searchImage</a> ";
			} else {
				print "\n <input type=\"hidden\" name=\"posb{$row['PEISEQ']}_{$row['PEENID']}\" id=\"posb{$row['PEISEQ']}_{$row['PEENID']}\" value=\"" . rtrim($row['PEPOSB']) . "\"><span $FldStyle>$row[PEPOSB]</span> ";
			}
			print "\n </td> ";
			// Pay Off Amount
			if ($row['PEPOAM']>0) {print "\n <td class=\"colnmbr\" id=\"poam{$row['PEISEQ']}_{$row['PEENID']}\">" . number_format($row['PEPOAM'],2) . "</td> ";}
			else                  {print "\n <td class=\"colnmbr\" id=\"poam{$row['PEISEQ']}_{$row['PEENID']}\">&nbsp;</td> ";}
		}
		...
		if ($columnDisplay['PEPOAM']!="Y") {print "\n <td><input type=\"hidden\" name=\"posb{$row['PEISEQ']}_{$row['PEENID']}\" id=\"posb{$row['PEISEQ']}_{$row['PEENID']}\" value=\"" . rtrim($row['PEPOSB']) . "\"></td> ";}
		print "\n </tr>";
