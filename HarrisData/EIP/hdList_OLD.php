...
if ($formatToPrint || $tag == "EXPORT") ini_set('max_execution_time', 1800); //300 seconds = 5 minutes
...
$linkB_count = 0; $linkB_sort = null;
$linkC_count = 0; $linkC_sort = null;
$linkD_count = 0; $linkD_sort = null;
$linkT_count = 0; $linkT_sort = null;
if ($hdDocsetLink && $hdListLink) {
	$linkB_count = count($hdDocsetLink->xpath("linkid[type='B']"));
	if ($linkB_count>0) {$linkB_sort = Link_Sort('B', $hdList_OPT); if (!$linkB_sort) {$linkB_count = 0;}}
	$linkC_count = count($hdDocsetLink->xpath("linkid[type='C']"));
	if ($linkC_count>0) {$linkC_sort = Link_Sort('C', $hdList_OPT); if (!$linkC_sort) {$linkC_count = 0;}}
	$linkD_count = count($hdDocsetLink->xpath("linkid[type='D']"));
	if ($linkD_count>0) {$linkD_sort = Link_Sort('D', $hdList_OPT); if (!$linkD_sort) {$linkD_count = 0;}}
	$linkT_count = count($hdDocsetLink->xpath("linkid[type='T']"));
	if ($linkT_count>0) {$linkT_sort = Link_Sort('T', $hdList_OPT); if (!$linkT_sort) {$linkT_count = 0;}}
}
// $uv_count = count($hdDocsetRow->xpath("col[user_view_col<>' ']"));

if ($hdListFilter)   {$viewCheckBoxDef = Rtv_Filter();}
if ($ovrRowsPerPage) {$dspMaxRows = $ovrRowsPerPage;}
...
if ($tag == "MASTERSEARCH") {
	...
	foreach ($srchCol as $colName) {
		...
		if ($coldType == "NUMERIC" || $coldType == "DATE") {
			...
			if ($colFormat == "CYMD" || $colFormat == "ISODATE") {$editVariables .= " chkUpper(document.Search.srch$colName) != 'TODAY' || editdate(document.Search.srch$colName,'$colHeading') ";}
			else {
	...
	if (!$editVariables) {require_once 'NoFormValidate.php';}
	else {
	...
	foreach ($srchCol as $colName) {
		...
		$maxWidth   = ($coldType == "DATE" || $colFormat == "CYMD" || $colFormat == "ISODATE") ? "6" : $colSize;
		if ($colDecm > 0) $maxWidth++;
		$colName    = str_replace("#", "_", $colName);
		if (!$focusField) {$focusField = "srch$colName";}
		...
		if ($colFormat == "PHONE") {
			...
			if ($colFormat == "CYMD" || $colFormat == "ISODATE") {print "\n <td>";  require "opersel_num_short.php"; print "</td>";}
			else {$selEqual="SELECTED"; print "\n <td>";  require "opersel_alph_short.php"; print "</td>";}
			print "\n <td class=\"inputnmbr\"><input type=\"text\" name=\"srch$colName\" size=\"$colWidth\" maxlength=\"$maxWidth\">";
...
if ($tag == "ORDERBY") {
	...
	if ($listCol[0]->label) {$colHeading = (string) $listCol[0]->label;} else {$colHeading = (string) $col[0]->label;}
	...
	if ($colRelID) {
		...
		if ($colRel3) {$sortFld3 = "({$colRel3})";}
		$colRel4 = (string) $colRel[0]->related_column_4;
		if ($colRel4) {$sortFld4 = "({$colRel4})";}
		$colRel5 = (string) $colRel[0]->related_column_5;
		if ($colRel5) {$sortFld5 = "({$colRel5})";}
	}
	if ($row[0]->label) {$colHeading = (string) $row[0]->label;} else {$colHeading = (string) $col[0]->label;}
	$colHeading = Format_Heading($colHeading);
	$colAltSort = (string) $col[0]->alt_sort;
	if (trim($colAltSort) == 'UPPER') {$colAltSort = "upper($colName)";}
	$sortFld = ($colAltSort)? $colAltSort : $colName;
	$sortFld = "({$sortFld})";
	$colHeading = ($colRelID)? $colRelDesc : $colHeading;
	if ($sortFld5)     {$orby = array(array($sortFld,"A",$colHeading),array($sortFld2,"A",""),array($sortFld3,"A",""),array($sortFld4,"A",""),array($sortFld5,"A",""));}
	elseif ($sortFld4) {$orby = array(array($sortFld,"A",$colHeading),array($sortFld2,"A",""),array($sortFld3,"A",""),array($sortFld4,"A",""));}
	elseif ($sortFld3) {$orby = array(array($sortFld,"A",$colHeading),array($sortFld2,"A",""),array($sortFld3,"A",""));}
	elseif ($sortFld2) {$orby = array(array($sortFld,"A",$colHeading),array($sortFld2,"A",""));}
	else               {$orby = array(array($sortFld,"A",$colHeading));}
...
if ($tag == "QSEARCH") {
	...
	if (is_null($_SESSION[$qsNameID]) || $_SESSION[$qsNameID] != $_POST['qsName'] || $_SESSION[$qsOperID] != $_POST['qsOper'] || $_SESSION[$qsValueID] != $_POST['qsValue']) {
		...
		if ($oper == "") {
			if ($coldType == "CHAR" || $coldType == "VARCHAR" || $coldType == "TIMESTMP") {$oper = "LIKE";} else {$oper = "=";}
		} elseif ($oper == "LIKE" && $coldType == "NUMERIC") {$coldType = "CHAR";}
		if     ($colFormat == "PHONE" && $oper == "LIKE")        {$returnValue = Build_WildCard($selName, $colHeading, $value, "U", $oper, "A");}
		elseif ($colFormat == "PHONE" && $coldType == "CHAR")    {$returnValue = Build_WildCard($selName, $colHeading, $value, "", "", "PA");}
		elseif ($colFormat == "PHONE")                           {$returnValue = Build_WildCard($selName, $colHeading, $value, "", "", "P");}
		elseif ($colFormat == "ISODATE" && $coldftVal == "NULL") {$returnValue = Build_WildCard($selName, $colHeading, $value, "", $oper, "IN");}
		elseif ($colFormat == "ISODATE")                         {$returnValue = Build_WildCard($selName, $colHeading, $value, "", $oper, "I");}
		elseif ($colFormat == "CYMD")                            {$returnValue = Build_WildCard($selName, $colHeading, $value, "", $oper, "D");}
		elseif ($colFormat == "CYR")                             {$returnValue = Build_WildCard($selName, $colHeading, $value, "", $oper, "CYR");}
		elseif ($colFormat == "PERIOD")                          {$returnValue = Build_WildCard($selName, $colHeading, $value, "", $oper, "DP");}
		elseif ($colFormat == "PERCENT")                         {$returnValue = Build_WildCard($selName, $colHeading, $value, "", $oper, "PCT");}
		elseif ($colFormat == "TIMEHHDD")                        {$returnValue = Build_WildCard($selName, $colHeading, $value, "", $oper, $colFormat);}
		elseif ($colFormat == "TIMESTAMP")                       {$returnValue = Build_WildCard($selName, $colHeading, $value, "", $oper, $colFormat);}
		elseif ($coldType  == "NUMERIC")                         {$returnValue = Build_WildCard($selName, $colHeading, $value, "", $oper, "N");}
		else                                                     {$returnValue = Build_WildCard($selName, $colHeading, $value, "U", $oper, "A");}
...
if ($tag == "WILDCARD") {
	...
	foreach ($srchCol as $colName) {
		...
		if ($oper == "LIKE" && $coldType == "NUMERIC") {$coldType = "CHAR";}
		if     ($colFormat == "PHONE" && $coldType == "CHAR")    {$returnValue = Build_WildCard($selName, $colHeading, $value, "", "", "PA");}
		elseif ($colFormat == "PHONE")                           {$returnValue = Build_WildCard($selName, $colHeading, $value, "", "", "P");}
		elseif ($colFormat == "ISODATE" && $coldftVal == "NULL") {$returnValue = Build_WildCard($selName, $colHeading, $value, "", $oper, "IN");}
		elseif ($colFormat == "ISODATE")                         {$returnValue = Build_WildCard($selName, $colHeading, $value, "", $oper, "I");}
		elseif ($colFormat == "CYMD")                            {$returnValue = Build_WildCard($selName, $colHeading, $value, "", $oper, "D");}
...
if ($tag != "EXPORT") {
	...
	require_once 'CalendarInclude.php';
	require_once 'DateEdit.php';
	if (!$formatToPrint) require_once 'Menu.js';
	require_once 'NumEdit.php';
	require_once 'CheckEnterSearch.php';
	if (!$editVariables) {require_once 'NoFormValidate.php';}
	else {
	...
	print "\n </head>";
	print "\n <body $bodyTagAttr>";
	if ($nMenu) {require_once ($searchBanner);} else {require_once 'Banner.php';}
	print "\n <table $baseTable>";
	print "\n <tr valign=\"top\">";
	$pageID = (string) $hdDocsetTable->name;
	if (!$nMenu) {require_once 'MenuDisplay.php';}
	print "\n <td class=\"content\">";
	print "<table $contentTable><colgroup><col width=\"80%\"> <col width=\"15%\"> \n <tr>";
	print "<td>";
	print "<h1>$pageHeading1</h1>";
	if ($pageHeading2) {print "<h2>$pageHeading2</h2>";}
	print "</td>";
	if (!$formatToPrint) {
		print "\n <td class=\"toolbar\">";
		if ($linkB_count>0) {dspLinks($linkB_sort);}
	...
	if ($sqlWhere) {
		...
		if ($_SESSION[$fKey2]) print $_SESSION[$fKey2];
		if ($_SESSION[$fKey3]) print $_SESSION[$fKey3];
		if ($_SESSION[$fKey4]) print $_SESSION[$fKey4];
		if ($_SESSION[$fKey5]) print $_SESSION[$fKey5];
		print "\n </table>";
	}
...
if (!isset($_SESSION['userViewCnt'])) {
	...
	$_SESSION['userViewCnt']=$userViewCnt;
}
if ($HDRUVP == 'Y' || $HDRUVP == 'L' && $_SESSION['userViewCnt'] > 0) {
	...
	if ($uv_Sql) {
		if ($sqlWhere) {$sqlWhere .= " and $uv_Sql";}
		else           {$sqlWhere = "Where $uv_Sql";}
	}
}
...
if ($hdListFilter) {
	...
	if ($viewCheckSQL) {
		if ($sqlWhere) {$sqlWhere .= " and $viewCheckSQL";}
		else           {$sqlWhere = "Where $viewCheckSQL";}
	}
}

if ($wildCardSearch && $sqlWhere) {$sqlWhere .= " $wildCardSearch";}
elseif ($wildCardSearch)          {$sqlWhere = "Where 1=1 $wildCardSearch";}

// Order By Clause
$sqlOrderBy = " Order By $orderBy";
$stmtSQL = $sqlSelect . $sqlFrom;
if ($sqlWhere) {$stmtSQL .= $sqlWhere;}
if ($sqlOrderBy) {$stmtSQL .= $sqlOrderBy;}
//if ($sqlGroupBy) {$stmtSQL .= $sqlGroupBy;}
if (!$pagingSelectList) {
	$sql_Record_Count=999999;
	if (!$formatToPrint && $tag != "EXPORT") {$fetchRows=$dspMaxRows+$startRow;} 
	else {$fetchRows=$prtMaxRows;} 
	$stmtSQL .= " Fetch First $fetchRows Rows Only ";
}
elseif ($formatToPrint == "") {$stmtSQL .= " For Fetch Only with NC Optimize For $dspMaxRows Rows ";}
else                          {$stmtSQL .= " For Fetch Only with NC Optimize For $prtMaxRows Rows ";}
if ($debug) print $stmtSQL;
...
if ($tag != "EXPORT") {
	if (!$formatToPrint) {
		...
		if ($pagingSelectList) {print " of $totalPages";}
		else {
        	$row = db2_fetch_assoc($sqlResult, $rowIndexNext);
        	if ($row == '') $sql_Record_Count=0;
		}
		if ($nextPrevPos != "2") {
			if ($startRow > $maxRows) {
		...
		if ($allowSaveFilter != "N") {
			...
			print "\n <a href=\"{$homeURL}{$phpPath}FilterSelection.php{$genericVarBase}&amp;tag=REPORT&amp;fromTblID=" . urlencode($tblID) . "&amp;fromPagID=" . urlencode($pagID) . "&amp;fromScript=" . urlencode($scriptName). "&amp;pageHeading1=" . urlencode($pageHeading1) . "\" onclick=\" saveFilterURL();{$selectionWinVar}\">{$wildSetImage}</a>";
		...
		foreach ($srchCol as $colName) {
			...
			if ($coldType == "DATE" || $colFormat == "CYMD" || $colFormat == "ISODATE") {$colID .= "|DATE";}
			else {$colID .= "|$colfType|$colHeading";}
			// if (strlen($colHeading)>20) {}
			if ($colName == $dftSrchColumn) {$selDft = "selected=\"selected\""; $selID=$colID;} else {$selDft = "";}
		...
		print "\n <input class=\"page\" type=\"text\" name=\"qsValue\" size=\"10\" maxlength=\"50\">";
	...
	if ($linkD_count>0 && !$formatToPrint) {print "<th class=\"colhdr\">$optionHeading</th>";}

	$seq = 0;
	foreach ($dspCol as $colName) {
		$listCol  = $hdListRow->xpath("col[@id='" . trim(strtoupper($colName)) . "']");
		$col  = $hdDocsetRow->xpath("col[@id='" . trim(strtoupper($colName)) . "']");
		if ($listCol[0]->colheading) {$colHeading = (string) $listCol[0]->colheading;} else {$colHeading = (string) $col[0]->colheading;}
		$colHeading = Format_Heading($colHeading);
		$colRel1  = (string) $col[0]->related_column_1;
		if ($colRel1) {
...
if ($tag == "EXPORT") {
	$xmlRootSfx   = "Document";
	$xmlNodeSfx   = "Row";
	$xmlContainer = str_replace(array(' ','#'), '', trim($xmlContainer));
...
while ($row = db2_fetch_assoc($sqlResult, $startRow)){
	...
	if ($tag == "EXPORT") {
		...
		$xmlID = $xmlDoc->createElement($xmlNodeName); $xmlRoot->appendChild($xmlID);
		foreach ($dspCol as $colName) {
			Get_Column();
			$colRel1    = (string) $col[0]->related_column_1;
			$colData    = trim($row[$colName]);
			if ($colRel1) $colData=Format_Related();
			$fmtHeading = str_replace(array(' ','#','/','<br>','&lt;br&gt;',':','%','(',')','!','@','^','*','+','=','{','}','[',']','$','%','&','~','`','|','?'), '', $colHeading);
			...
			$fmtValue   = $colData;
			if ($coldType == "NUMERIC" || $coldType == "DATE") {
				if     ($colFormat == "CYMD")    {$fmtValue = Format_Date($colData, "D");}
				elseif ($colFormat == "ISODATE") {$fmtValue = Format_Date_ISO($colData, "D");}
			}
			$xmlTag = $xmlID->appendChild($xmlDoc->createElement($fmtHeading));  $xmlTag->appendChild($xmlDoc->createTextNode($fmtValue));
		}
		$startRow ++;
		$rowCount ++;
		continue;
				...
				if ($optPOPT == "00" || (!$optPGOV && $hdList_OPT["sec_$optPOPT"] == "Y") || ($optPGOV && $ovp_OPT["sec_$optPOPT"] == "Y")) {
					...
					if ($optSELC != "") {
						...
						if ($optFILN) {
							...
							if ((int)$testCond==false) {$rcdCnt = 0;}
						}
					}
					if ($rcdCnt) {
						$saveDSEQ = $optDSEQ;
						...
						if (trim($optIMG) == "") {$optIMG  = (string) $col[0]->image;}
						...
						if (strpos($optURL, "@@confirm") !== false) {
							...
							foreach ($hdListRow->col as $listCol) {
								...
								if ($colRel1) $colData=Format_Related ();
								$fmtValue   = $colData;
								$fmtValue   = str_replace(array("'",'"',"&"), array("&acute;", "&quot;","&amp;"), $colData);
						...
						if ($optD2WN || $optURL) {
							...
							$optURL .= $maintainVar;
							if ($optTRGT != "") {
								if     (strtoupper($optTRGT) == "COMMENT"){$tgt = " onclick=\"$commentWinVar\" ";}
								elseif (strtoupper($optTRGT) == "INQUIRY"){$tgt = " onclick=\"$inquiryWinVar\"";}
								elseif (strtoupper($optTRGT) == "INVOICE"){$tgt = " onclick=\"$invoiceWinVar\"";}
								else                                      {$tgt = " target=\"$optTRGT\"";}
	...
	foreach ($dspCol as $colName) {
		...
		if ($colName != $savName) {
			...
			if ($colfType != '') {
				$selData = $colData;
				if ($coldType == "NUMERIC" && $colSize > 1) {$selData = str_pad($colData, $colSize, "0", STR_PAD_LEFT);}
				$toolTip = RetValue("FLTYPE='$colfType' and FLVALU='$selData'", "SYFLAG", "FLDESC");
			}
			elseif ($colFormat == "CYMD" || $colFormat == "ISODATE") {
				if ($colFormat == "CYMD") {$wrkDate = Date_CYMD_ISO($colData);} else {$wrkDate = $colData;}
			...
			if ($colFormat == "CREDITCARD") {$colData = "";}

			$optIMG  = null;
			$colLink = false;
			$condErr = false;
			$rcdCnt  = 0;
			if ($colRel1) {$linkCol=$colRel1;} else {$linkCol=$colName;}
			if (!$formatToPrint) {
				...
				if ($linkColSort) {
					...
					foreach ($linkColSort as $linkID) {
						...
						foreach ($hdDocsetLink->xpath("linkid[@id='" . trim($linkID) . "']") as $link_data) {
							...
							if (trim($optIMG) == "") {$optIMG  = (string) $link_data->image;}
							...
							if ($colCond) {
								...
								eval("\$testCond = " . trim($colCond).";");
								if ((int)$testCond==false) {$condErr = true;}
							}
							if (trim($optURL) == "" && strpos($optD2WN, "@@parm") !== false) {
								...
								$optURL = $optD2WN;
								$optD2WN="";
								if (trim($optTRGT) == "") {$optTRGT  = "INQUIRY";}
							}
							if (!$condErr) {
								$rcdCnt = 1;
								if ($optSELC != "") {
									...
									if ($optFILN) {$rcdCnt = RetValue("$optSELC", "$optFILN", "char(count(*))");}
									else {
										$optSELC = str_replace(" and ", " && ", $optSELC);
										$optSELC = str_replace(" or ", " || ", $optSELC);
										$optSELC = str_replace("&lt;", "<", $optSELC);
							...
							if ($rcdCnt) {
								$colLink = true;
								if ($optTRGT == "")                        {$tgt = "";}
								elseif (strtoupper($optTRGT) == "COMMENT") {$tgt = " onclick=\"$commentWinVar\" ";}
								elseif (strtoupper($optTRGT) == "INQUIRY") {$tgt = " onclick=\"$inquiryWinVar\" ";}
								elseif (strtoupper($optTRGT) == "INVOICE") {$tgt = " onclick=\"$invoiceWinVar\" ";}
								elseif ($optTRGT != "")                    {$tgt = " target=\"$optTRGT\" ";}
			...
			$savRelID = $colRelID;
			if ($colFormat != '') {
				if (!$toolTip && $colFormat != "SSN") {$toolTip=$colData;}
				$colData=$fmtValue;
			}
			if ($optIMG) {$cssClass = "colcode";}
			$colTitle = str_replace("&nbsp;", " ", $colData);
			print "\n <td class=\"$cssClass\" title=\"" . htmlentities($colTitle, ENT_QUOTES) . "\">";
			if ($colLink) {
				$optURLSearch   = array();
				$optURLSearch[] = "@@homeURL";
				$optURLSearch[] = (strpos($optURL, "@@phpPath") !== false) ? "@@phpPath" : "@@cGIPath";
				...
				$optPARM = str_replace("&", "&amp;", $optPARM);
				$optURL .= $optPARM;
				if ($optIMG) {$fmtValue = ${$optIMG};}
				if ($toolTip) {$moreInfo = "\n$toolTip";} else {$moreInfo = "";}
				$moreInfo = htmlentities($moreInfo, ENT_QUOTES);
				$colData  = htmlentities($colData, ENT_QUOTES);
				if (trim(htmlentities($moreInfo, ENT_QUOTES)) == trim($colData)) $moreInfo = "";
			...
			elseif ($toolTip) {print "<span $helpCursor title=\"$toolTip\">$fmtValue</span>";}
			else              {print "$fmtValue";}
			print "</td>";
		}
	}
...
if ($dspTot) {
	require 'SetRowClass.php';
	print "\n <tr>";
	if ($linkD_count>0 && !$formatToPrint) {print "<td>&nbsp;</td>";}
	$sumSel="";
	...
	if ($obPOS) $newSQL = substr($newSQL, 0, $obPOS-1);
...
print "\n </td> </tr> </table>";
if ($nMenu) {require_once ($searchTrailer);} else {require_once 'Trailer.php';}
print "\n </body> \n </html>";
?>