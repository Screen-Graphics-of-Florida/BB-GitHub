...
try {
    $conn = ToolkitService::getInstance('*LOCAL', 'CHUTCH', 'CHUTCH');
} catch (Exception $e) {
...
echo 'Before parameter build: ' . '<br>';
echo 'Profile Handle: ' . $profileHandle . '<br>';
echo 'Database ID: ' . $databaseID . '<br>';
echo 'Customer Number: ' . $custNumber . '<br>';
echo 'Item Number: ' . $itemNumber . '<br>';
echo 'Price: ' . $price . '<br>';
echo 'Warehouse: ' . $warehouse . '<br>';
echo 'Cost Plus: ' . $costPlus . '<br><br>';


// Add Parameters for calling HD's pricing routine
$params = [];    // Start with an empty array
$params[] = $conn->AddParameterChar('both', 64, 'Profile Handle', 'HEXHND', $profileHandle);
$params[] = $conn->AddParameterChar('both', 2, 'databaseID', 'DBID', $databaseID);
$params[] = $conn->AddParameterZoned('both', 7, 0, 'Customer Number', 'CUST', $custNumber);
$params[] = $conn->AddParameterChar('both', 15, 'Item Number', 'ITEM', $itemNumber);
$params[] = $conn->AddParameterZoned('both', 13, 5, 'Price', 'PRICE', $price);
$params[] = $conn->AddParameterZoned('both', 3, 0, 'warehouse', 'WHS', $warehouse);
$params[] = $conn->AddParameterZoned('both', 9, 4, 'Cost Plus', 'PCPP', $costPlus);


// Define a return parameter.  Can be any type, even a data structure.
$retParam = $conn->AddParameterInt32('out', '4-byte int', 'MYRESULT', '13579');

// --------------------> THIS LINE IS THE ONE THAT FAILS!!! <------------------------
/*
$result = $ToolkitServiceObj->PgmCall('HOEPRC_W', 'HDSSTDPGM', $params, $retParam, null); 
...
echo 'Input/Output params:<br>';
echo 'Profile Handle: ' . $result['io_param']['HEXHND'] . '<br>';
echo 'Database ID: ' . $result['io_param']['DBID'] . '<br>';
echo 'Customer: ' . $result['io_param']['CUST'] . '<br>';
echo 'Item Number: ' . $result['io_param']['ITEM'] . '<br>';
echo 'Price: ' . $result['io_param']['PRICE'] . '<br>';
echo 'Warehouse: ' . $result['io_param']['WHS'] . '<br>';
echo 'Cost Plus: ' . $result['io_param']['PCPP'] . '<br><br>';
