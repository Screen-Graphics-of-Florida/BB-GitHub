...
$page_title            = "G/L Budget Review Sheet";
$scriptName            = "GLBudgetReviewSheet.php";
...
if ($tag == "REPORT") {
	...
	print "\n    && editNum(document.Chg.descriptor, 2, 0) ";
	...
	print "\n    && editFromToAll(document.Chg.frPeriod, document.Chg.toPeriod, document.Chg.allPeriod, 2) ";
	print "\n    ) {return true;} ";
	print "\n } ";
...
?>
  ...
  // Descriptor
  function OpenApplyDescriptor() {showSel('showApplyDescriptor');}  
  function CloseApplyDescriptor() {hideSel('showApplyDescriptor');} 
  function editAPLY() { 
    if (document.getElementById('applyDescriptor').checked) {OpenApplyDescriptor();}
    else {CloseApplyDescriptor();}
  } 
...
while ($rowBdg = db2_fetch_assoc($sqlResult, $startRow)){
	if ($rowBdg) {$budgets[] = array("sequence"=>0,"plan"=>$rowBdg['BGNMBR'],"desc"=>$rowBdg['BGDESC']);}
	$startRow ++;
}
...
if ($errFound != "" || $scheduleJobSwitch == "Y") {
	...
	if ($errFound != "") {
		...
		$Err_APER=DecatErr_Field("@@aper", "allPeriod");
	...
	foreach ($col_sort as $colID) {
		...
		for ($i=0;$i<count($budgets);$i++) {if($budgets[$i]["plan"] == $BDG) {$budgets[$i]["sequence"] = $SEQ;}}
	}
...
if ($ACO == "ALL") {$checked_ACO="CHECKED";} else {$checked_ACO="";}
...
// Apply Descriptor Table
$fldChecked=Field_Checked($APLY,"Y");
$textOvr=SetTextOvr($Err_APLY);
print "\n <tr><td class=\"dsphdr\"><span $textOvr>Apply Descriptor Table</span></td> ";
print "\n     <td class=\"inputalph\"><input type=\"checkbox\" name=\"applyDescriptor\" id=\"applyDescriptor\" value=\"Y\" $fldChecked onClick=\"editAPLY()\"></td>";
print "\n </tr> ";
DspErrMsg($Err_APLY);
...
// Descriptor Table (hidden DIV)
print "\n <div id=\"showApplyDescriptor\">";
print "\n <fieldset class=\"legendBody\"> ";
print "\n <legend class=\"legendTitle\">Apply Descriptor Table ";
print "\n </legend> ";
print "\n <table $contentTable> ";
$textOvr=SetTextOvr($Err_DSCR);
print "\n <tr><td class=\"dsphdr\"><span $textOvr>Descriptor Table</span></td> ";
print "\n     <td class=\"inputnmbr\"><input type=\"text\" name=\"descriptor\" value=\"" . rtrim($DSCR) . "\" size=\"2\" maxlength=\"2\"> ";
print "\n                             <a href=\"{$homeURL}{$phpPath}GLDescriptorSearch.php{$genericVarBase}&amp;tag=REPORT&amp;docName=Chg&amp;fldName=descriptor\" onclick=\"$searchWinVar\"> $searchImage</a></td>";
print "\n </tr> ";
DspErrMsg($Err_DSCR);
print "\n </table> ";
print "\n <script TYPE=\"text/javascript\">";
if (trim($APLY)=="Y") {print "\n OpenApplyDescriptor()";}
else                  {print "\n CloseApplyDescriptor()";}
print "\n </script>";
print "\n </fieldset> ";
print "\n </div>";
...
Print "\n </legend> ";
print "\n <table $contentTable> ";
$fieldDesc=(empty($PLAN)) ? '' : RetValue("BGNMBR=$PLAN", "GLBGHD", "BGDESC");
...
print "\n         <tr><td class=\"dsphdr\"><span $textOvr>Period</span></td> ";
...
print "\n             <td class=\"inputalph\"><input type=\"checkbox\" name=\"allPeriod\" value=\"ALL\" $checked_APER onClick=\"if (this.checked) this.form.frPeriod.value='', this.form.toPeriod.value='';\"></td> ";
print "\n         </tr> ";
DspErrMsg($Err_FPER);
DspErrMsg($Err_TPER);
DspErrMsg($Err_APER);
...
if ($tag == "Edit_Data") {
	$edtVar= "";
	if (!isset($_POST['consolReport'])) {$_POST['consolReport']="N";}  Concat_Field("@@cons", $_POST['consolReport']);
	if (!isset($_POST['incBalSheet'])) {$_POST['incBalSheet']="N";}  Concat_Field("@@bals", $_POST['incBalSheet']);
	if (!isset($_POST['applyDescriptor'])) {$_POST['applyDescriptor']="N";}  Concat_Field("@@aply", $_POST['applyDescriptor']);
	if (!isset($_POST['newBudget'])) {$_POST['newBudget']="N";}  Concat_Field("@@newb", $_POST['newBudget']);
	if (!isset($_POST['incNotPlan'])) {$_POST['incNotPlan']="N";}  Concat_Field("@@notp", $_POST['incNotPlan']);
	Concat_Field("@@year", $_POST['fiscalYear']);
	Concat_Field("@@cnsd", $_POST['consolDate']);
	Concat_Field("@@dscr", $_POST['descriptor']);
	...
	if (!isset($_POST['allCompany'])) {$_POST['allCompany']="";}  Concat_Field("@@aco@", $_POST['allCompany']);

	Concat_Field("@@ffac", $_POST['frFacility']);
	Concat_Field("@@tfac", $_POST['toFacility']);
	if (!isset($_POST['allFacility'])) {$_POST['allFacility']="";}  Concat_Field("@@afac", $_POST['allFacility']);

	Concat_Field("@@facc", $_POST['frAccount']);
	Concat_Field("@@tacc", $_POST['toAccount']);
	if (!isset($_POST['allAccount'])) {$_POST['allAccount']="";}  Concat_Field("@@aacc", $_POST['allAccount']);

	Concat_Field("@@fsub", $_POST['frSubAcct']);
	Concat_Field("@@tsub", $_POST['toSubAcct']);
	if (!isset($_POST['allSubAcct'])) {$_POST['allSubAcct']="";}  Concat_Field("@@asub", $_POST['allSubAcct']);

	Concat_Field("@@fper", $_POST['frPeriod']);
	Concat_Field("@@tper", $_POST['toPeriod']);
	if (!isset($_POST['allPeriod'])) {$_POST['allPeriod']="";}  Concat_Field("@@aper", $_POST['allPeriod']);
	...
	$returnValue=Selection_Edit_Handle("HGLBRS_W", $profileHandle, $dataBaseID, $submitSchedule, $errFound, $edtVar, $errVar, $wrnVar);
