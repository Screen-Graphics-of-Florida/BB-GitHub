...
if ($activeRole == "") {
    ...
    if ($portalByRoleCnt) {
        $fileSQL .= "  inner join SYPORR on RDROLE=PRROLE and FPPORT=PRPORT and FPPAGE=PRPAGE and FPSEQ=PRSEQ ";
    }
    ...
    if ($portalByRoleCnt) {
        $selectSQL .= " and PRSEL='Y' ";
    }
    $stmtSQL .= " From $fileSQL";
    if (trim($selectSQL) != "") {
        $stmtSQL .= " Where {$selectSQL}";
    }
    ...
    while ($row = db2_fetch_assoc($sqlResult)) {
        if ($row == "0") {
            ...
            if ($V_FUTRGT == "") {
                $tgt = "";
            } elseif (strtoupper($V_FUTRGT) == "COMMENT") {
                $tgt = " onclick=\"$commentWinVar\"";
            } elseif (strtoupper($V_FUTRGT) == "INQUIRY") {
                $tgt = " onclick=\"$inquiryWinVar\"";
            } elseif (strtoupper($V_FUTRGT) == "INVOICE") {
            ...
            if ($poshomeURL >= 0) {
                ...
                if ($pos) {
                    $workURL = str_replace("@@browser", $browser, $workURL);
                }
            ...
            if ($V_FPPAGE != "") {
                ...
                if ($svpage == "" || $svpage != $V_FPPAGE && $V_FPPAGE != $pageID) {
                    if ($endUL) {
                        print "\n </ul>";
                    }
                    $endUL = 1;
            ...
            if ($V_FUURL == "") {
                if (($V_FUIMG != "") and ($displayMenuImages == "Y")) {
