                                     ...
                                     OUT CHAR(600) F_confirmDesc)
{
  ...
  %if (confirmDesc2 != "")
      ...
      @dtw_concat(F_confirmDesc, "\n$(confirmDesc2) $(F_confirmData2)", F_confirmDesc)
  %endif
  %if (confirmDesc3 != "")
      @Format_Code(@DTW_rSTRIP(confirmData3), F_confirmData3)
      @dtw_concat(F_confirmDesc, "\n$(confirmDesc3) $(F_confirmData3)", F_confirmDesc)
  %endif
                                 ...
                                 OUT CHAR(100) F_EmpName)
{
  ...
  %if (firstName != "" || lastName != "")
      ...
      %elif (termCode != "" && termHD == "D")
          @dtw_concat(F_EmpName, "&nbsp; $(termEmplDetail)", F_EmpName)
      %endif
  %endif
%}

%MACRO_FUNCTION Format_SSN (IN  CHAR(9) ssnIn,
                            OUT CHAR(12) F_ssnIn)
{
  @dtw_assign(F_ssnIn, "")
  %while(@dtw_rlength(ssnIn) != "9") {@dtw_insert("0", ssnIn, ssnIn)%}
  %if (ssnIN != "000000000")
     @dtw_assign(F_ssnIn, ssnIn)
     @dtw_insert("-", F_ssnIn, "3", "1", F_ssnIn)
     @dtw_insert("-", F_ssnIn, "6", "1", F_ssnIn)
  %endif
%}
                              ...
                              INOUT CHAR(10) dateOut)
{
 %if (dateIn != "" && dateIn != "0001-01-01")
     %if (sysDateFormat == "YMD")
         @dtw_assign(dateOut, "@dtw_rsubstr(dateIn, "3", "2")@dtw_rsubstr(dateIn, "6", "2")@dtw_rsubstr(dateIn, "9", "2")")
     %elif (sysDateFormat == "DMY")
         @dtw_assign(dateOut, "@dtw_rsubstr(dateIn, "9", "2")@dtw_rsubstr(dateIn, "6", "2")@dtw_rsubstr(dateIn, "3", "2")")
     %else
...
%MACRO_FUNCTION DateInputFromISO  (IN    CHAR(10) dateIn,
                                   INOUT CHAR(10) dateOut)
{
 %if (dateIn != "" && dateIn != "0001-01-01")
     %if (sysDateFormat == "YMD")
...
%MACRO_FUNCTION Date_To_ISO (IN    CHAR(10) dateIn,
                              INOUT CHAR(10) dateOut)
{
 %if (dateIn == "0")
     @dtw_assign(dateOut, "0001-01-01")
...
%MACRO_FUNCTION YearFromCYY (INOUT year) {
  %if (year > "0")
      %while(@dtw_rlength(year) != "3") {@dtw_insert("0", year, year)%}
      @dtw_add(year, "1900", year)
  %endif
%}
...
%MACRO_FUNCTION Format_Decmin (IN  CHAR(4) decminIn,
                               OUT CHAR(4) F_decminIn)
{
  @dtw_assign(F_decminIn, "")
  @dtw_pad("L", decminIn, @dtw_rsubtract("4", @dtw_rlength(decminIn)), "0", decminIn)
  @dtw_substr(decminIn,"3","2",dec)
  @dtw_substr(decminIn,"1","2",min)
  @dtw_multiply(dec,"0.6","2",calcDec)
  @dtw_insert(@dtw_rpad("L", min, @dtw_rsubtract("2",@dtw_rlength(min)),"0"), F_decminIn, "0", F_decminIn)
  @dtw_insert(@dtw_rpad("L", calcDec, @dtw_rsubtract("2",@dtw_rlength(calcDec)),"0"), F_decminIn, "2", F_decminIn)
%}
...
%MACRO_FUNCTION Build_More_Info (IN  CHAR(100) fldName,
                                 IN  CHAR(100) fldDesc,
                                 IN  CHAR(100) fldValue,
                                 INOUT CHAR(32000) moreInfo)
{
    %if (fldName != "")
       @dtw_concat(fldName, ":", fldName)
    %else
       @dtw_assign(fldName, "&nbsp;")
    %endif

    @dtw_concat(moreInfo, "<tr><td class=""hdrtitl"">$(fldName)</td><td class=""hdrdata"">$(fldDesc) &nbsp; $(fldValue)</td></tr>", moreInfo)
%}				

%MACRO_FUNCTION CalcDateWithOffset (INOUT fromField) {
      @dtw_uppercase(fromField,fromUpper)
      @dtw_pos("TODAY",fromUpper,posToday)
      %if (posToday > "0")
        @dtw_pos("-",fromUpper,possub)
        @dtw_pos("+",fromUpper,posadd)
        @dtw_assign(addSub, "")
        %if (possub > "0")
          @dtw_assign(addSub, "-")
        %elseif (posadd > "0")
        ...
        %endif
        %if (addSub != "")
            @dtw_pos("YEAR",fromUpper,posY)
            @dtw_pos("MONTH",fromUpper,posM)
            @dtw_pos("WEEK",fromUpper,posW)
            %if (posY > "0")
              @dtw_assign(ymd, "Y")
            %elseif (posM > "0")
              @dtw_assign(ymd, "M")
            ...
            @dtw_assign(x, "1")
            @dtw_assign(duration, "")
            %while(x <= @dtw_rlength(fromUpper)) {
               @dtw_substr(fromUpper, x, "1", testChar)
               %if (testChar >= "0" && testChar <= "9")
                  @dtw_concat(duration, testChar, duration)
               %endif
               @dtw_add(x, "1", x)
            %}
            @dtw_DATE("U", calcDate) @Date_MDY_ISO(@dtw_rReplace(calcDate,"/",""), calcDate)

            %if (posW > "0")
                @dtw_multiply(duration, "7", duration)
            %endif

            @Calc_Date_Offset(calcDate, addSub, duration, ymd)
            @Date_ISO_MDY(calcDate, fromField)
        %else
            @DateTodayCYMD(fromField)
            @DateInputFromCYMD(fromField)
        %endif
      %endif
%}
