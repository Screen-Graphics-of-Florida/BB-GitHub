<?php
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// DVG - Update to get pricing to work. from 5-06-2023 backup
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
//            print "\n <td class=\"colcode\">" .Format_Nbr($row['IWRESQ'], '0', $amtEditCode, 'Y', '', ''). "</td></tr>";
            ...
            print "\n <tr><td class=\"colcode\"><a href=https://portal.screen-graphics.com:5601/harris-CGI/ItemWarehouseSelect.d2w/REPORT?baseVar=BaseConfiguration.icl&portal=ITEM&fromItemNumber=$itemNumber&fromWarehouse=$F_IWWHS&itemNumber=$itemNumber&warehouseNumber=$F_IWWHS target=_blank>" .Format_Nbr($row['IWOHQT'], '0', $amtEditCode, 'Y','', ''). "</a></td>";
            print "\n <td class=\"colcode\"><a href=https://portal.screen-graphics.com:5601/harris-CGI/ItemPlantSelect.d2w/REPORT?baseVar=BaseConfiguration.icl&portal=ITEM&fromItemNumber=$itemNumber&fromPlant=".$row['IPPLT']."&itemNumber=$itemNumber&plantNumber=".$row['IPPLT']." target=_blank>" .Format_Nbr($row['IPCMTO'], '0', $amtEditCode, 'Y', '', ''). "</a></td>";
            print "\n <td class=\"colcode\"><a href=https://portal.screen-graphics.com:5601/harris-CGI/ItemPlantSelect.d2w/REPORT?baseVar=BaseConfiguration.icl&portal=ITEM&fromItemNumber=$itemNumber&fromPlant=".$row['IPPLT']."&itemNumber=$itemNumber&plantNumber=".$row['IPPLT']." target=_blank>" .Format_Nbr($row['IPQMRL'], '0', $amtEditCode, 'Y', '', ''). "</a></td>";
            print "\n <td class=\"colcode\">" .Format_Nbr($row['IWQOPK'], '0', $amtEditCode, 'Y', '', ''). "</td>";
            print "\n <td class=\"colcode\"><a href=https://portal.screen-graphics.com:5601/hdList.php?tblID=184&baseVar=BaseConfiguration.php&portal=CUSTOMER&pagID=114&tag=QSEARCH&downloadToCsv= target=_blank>" .Format_Nbr($row['IWRESQ'], '0', $amtEditCode, 'Y', '', ''). "</a></td></tr>";
...
// DVG
    ...
    $stmtSQL .= " PMPMKY, PMSTDT, PMEXDT, PMLCAM, PMLCPC, PBSTDT, PBLMT, PBPRC, MESSAGE ";
    $stmtSQL .= " FROM Table(UDF_PriceInquiry('$itemNumber', '$custClass'))";
    $stmtSQL .= " WHERE PMCCLS = '$custClass'  or PMCCLS = ' ' ";
    
    $lvlResult = db2_exec($i5Connect->getConnection (), $stmtSQL, array('cursor' => DB2_SCROLLABLE) );
    while ($row = db2_fetch_assoc($lvlResult) and $found == "No") {
...
//        $Price        = 0;   
        $message      = $row['MESSAGE'];
        $PriceLLPA    = $row['PMLCAM'];  // List Less/Cost Plus Amount
        $PriceLLPP    = $row['PMLCPC'];  // List Less/Cost Plus Percentage
        $PriceCom     = $row['PMCMPC'];  // Commission Percentage
        $PriceLimit   = $row['PBLMT'];   // Bracket Limit
        $PriceBracket = $row['PBPRC'];   // Bracket Price

// DVG   
        if (!empty($custClass)) {$Price = $PriceLLPA * $reqQty;}
        
        if ($priceDefDsp != 1) {
// DVG - TEMP Fix for bracket pricing is to set $found to No
            if ($message == "Active") {$found = "Yes";} 
            $priceDefDsp = 1;
...
// DVG            if ($listLess == "Y")   {$structureDefn = "List Less";}
            if ($listLess == "Y")   {$structureDefn = "Discount";}
        ...
        } // End the Pricing Definition Display
        ...
        if ($bracketQty != "Y" && $bracketAmt != "Y")  {
            if ($usePercent == "Y") {
		      ...
		      if ($listLess == "Y") {
		          ...
		          $singlePrice = $F_IWLIST - ($F_IWLIST * $PriceLLPP);
		          print "\n <tr><td>&nbsp</td></tr>";
		          ...
		          print "\n <tr><td>&nbsp</td</tr>";
                  print "\n <tr><td class=\"hdrtitl\"><span $textOvr>$structureDefn</span></td>";
                  print "\n     <td class=\"hdrdata\" size=\"20\" maxlength=\"14\">$" .Format_Nbr($PriceLLPA, '2', $amtEditCode, 'Y', '', ''). "</td>";
		          print "\n </tr>";
	           ...
	           if ($commission == "Y" || $commission == "L") {
		          ...
		          print "\n     <td class=\"inputnmbr\"><input type=\"text\"  name=\"commPercent\" value=\"" . rtrim($PriceCom) . "\" size=\"20\" maxlength=\"8\"></td>";
		          print "\n </tr>";
        ...
        ////////////////////////////////////
            ...
            if ($PriceBracket != 0.00) {
	               
	          // Break out units into an array to see are there setup costs...
	            $units = explode(".", $PriceLimit);
           
// Display setup costs if there are any...
	           if ($units[1] > 0 && $firstPass == 'Y') {
	               print "\n <tr><td class=\"dsphdr\"><span $textOvr>Setup Costs</span></td>";
	               print "\n      <td class=\"colcode\" size=\"20\" maxlength=\"14\">" .Format_Nbr($PriceLimit, '2', $amtEditCode, 'Y', '', ''). "</td>";
	               print "\n      <td class=\"colcode\" size=\"20\" maxlength=\"14\">$" .Format_Nbr($PriceBracket, '5', $amtEditCode, 'Y', '', ''). "</td>";
	          	   print "\n </tr>";
	          	    // If there are setup costs, use that price for least quantity...
	          	   $Price = $PriceBracket * $reqQty;
	          	   $firstPass = 'N';
...
// There are no setup costs, use max less than requested quantity price * quantity or == price if quantity matches requested quantity...
	               print "\n  <tr><td>&nbsp</td><td class=\"colcode\" size=\"20\" maxlength=\"14\">" .Format_Nbr($PriceLimit, '0', $amtEditCode, 'Y', '', ''). "</td>";
	               print "\n      <td class=\"colcode\" size=\"20\" maxlength=\"14\">$" .Format_Nbr($row['PBPRC'], '2', $amtEditCode, 'Y', '', ''). "</td>";
	               print "\n </tr>";
	               $intPBLMT = intval($PriceLimit);
	               $intreqQty = intval($reqQty);
	               if ($intPBLMT == $intreqQty && $firstPass == 'Y') { $Price = $PriceBracket; }
	               if ($intPBLMT == $intreqQty && $firstPass != 'Y') { $Price = $PriceBracket * $reqQty; }
	               if ($intPBLMT < $intreqQty) { $Price = $PriceBracket * $reqQty; }
	               $firstPass = 'N';
	             // print "\n <tr><td>Got here!</td><td>$Price</td></tr>";
	           }
...
// DVG    }  // End of Do-While Loop
            
    if ($found == "No") {
        if ($message == "Expired") {print "<b>Pricing has Expired!</b>";} 
...
// DVG   $rowCount = 0;
   $stmtSQL = "";
   $stmtSQL .= " Select HHORTY, HHBDTE, DHORD#, DHORL#, DHORCS, DHQORD, DHLIST, DHSLPR, HHSHTO ";
   ...
   $historyTitle = 'Customer / Item Order History';
} else {
    $stmtSQL = "";
    $stmtSQL .= " Select HHORTY, HHBDTE, DHORD#, DHORL#, DHORCS, DHQORD, DHLIST, DHSLPR, HHSHTO ";
    $stmtSQL .= " From OEORDH Left Outer Join OEORHH on DHORD# = HHORD# And DHSEQ# = HHSEQ# ";
    $stmtSQL .= " Where DHITEM = '$itemNumber' And DHSEQ# = 0 ";
    $stmtSQL .= " Order By HHBDTE DESC Fetch First 10 rows only ";
    $historyTitle = 'Item Order History';
}
$rowCount = 0;
$sqlResult = db2_exec($i5Connect->getConnection (), $stmtSQL);
while($row = db2_fetch_assoc($sqlResult)) {
     ...
     if ($custItemHistHdrs != "Displayed"){
        print "<table align=\"left\" hspace=\"30\" frame=\"box\" cellspacing=\"1\" cellpadding=\"0\">";
        print "\n <tr><td colspan=\"100%\" bgcolor=\"#000000\" p style=\"color:#FFFFFF\" align=\"center\"><b>$historyTitle</b></td></tr>";
       // print "\n <tr><td colspan=\"100%\" bgcolor=\"#FFFFFF\" p style=\"color:#FFFFFF\">&nbsp</td></tr>";
        ...
        if ($historyTitle != 'Item Order History'){
            print "\n <th class=\"colhdr\">Selling<br>Price</th>";
            print "\n <th class=\"colhdr\">Total<br>Sale</th>";
        }
        print "\n </tr>";
     ...
     print "\n <tr>";
     print "\n <td class=\"colcode\">" .$row['HHSHTO']. "</td>";
