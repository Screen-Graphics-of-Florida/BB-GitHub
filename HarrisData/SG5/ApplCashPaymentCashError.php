...
$scriptName    = "ApplCashPaymentCashError.php";
...
if ($tag == "ORDERBY"){
	...
	elseif ($sequence == "PayOffSubCode") {$orby = array(array("PEPOSB" ,"A","Pay Off Payment Code"),array("PESINV" ,"A","Invoice"));}
	elseif ($sequence == "PayOffAmount")  {$orby = array(array("PEPOAM" ,"A","Pay Off Amount"),array("PESINV" ,"A","Invoice"));}
...
require 'stmtSQLClear.php';
$stmtSQL .= " Select PEISEQ,PEENID,PESSEQ,PEAMT,PEDAMT,PEPOSB,PECMNT,PEPOTP,PEPOEN ";
...
while ($row = db2_fetch_assoc($sqlResult, $startRow)){
	...
	if ($applCashPaymentDeletePrompt=="Y") {
		if (trim($row['PEPOSB'])>"") {$deleteMsg="Payment and Payoff for invoice {$row[IVAINV]}";}
		else                         {$deleteMsg="Payment for invoice {$row[IVAINV]}";}
	...
	if ($columnDisplay['PEPOAM']=="Y") {
		...
		if ($harced_OPT['sec_03']=="Y" || $harced_OPT['sec_06']=="Y" && $CRDPYC!="") {
			print "\n     <input type=\"text\" name=\"posb{$row['PEISEQ']}_{$row['PEENID']}\" id=\"posb{$row['PEISEQ']}_{$row['PEENID']}\" value=\"" . rtrim($row['PEPOSB']) . "\" size=\"4\" maxlength=\"4\" $FldStyle onBlur=\"editPEPOSB('$row[PEISEQ]','$row[PEENID]','$row[MINPEENID]')\"> ";
			...
			print "\n     <input type=\"hidden\" name=\"posb{$row['PEISEQ']}_{$row['PEENID']}\" id=\"posb{$row['PEISEQ']}_{$row['PEENID']}\" value=\"" . rtrim($row['PEPOSB']) . "\" $FldStyle>$row[PEPOSB] ";
		}
		print "\n </td> ";
	...
	if ($columnDisplay['PEPOAM']!="Y") {print "\n <td><input type=\"hidden\" name=\"posb{$row['PEISEQ']}_{$row['PEENID']}\" id=\"posb{$row['PEISEQ']}_{$row['PEENID']}\"></td> ";}
	print "\n </tr>";
