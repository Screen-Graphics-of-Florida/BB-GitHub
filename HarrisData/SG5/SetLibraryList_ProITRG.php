...
/********************************************************************/
/* Chris Hutchinson - Pro I.T. Resource Group, Inc.                 */
/* chutchinson@proitrg.com  |  www.proitrg.com  |  1-800-545-7630   */
/*                                                                  */
/* March 20, 2014 (Created)                                         */
/*                                                                  */ 
/* This is a direct copy of SetLibraryList.php from /HarrisData/EIP */
/* that includes debug statements so we can see what's going on w/o */
/* affecting production when testing in the production database.    */
/********************************************************************/


require_once '/usr/local/zendsvr/share/ToolKitAPI/CW/cw.php';
...
/* 0001 - Chris Hutchinson   */
echo "Library List:</br>";
print_r ($libraryList). "</br";
/*****************************/
...
if ($eUser != $userProfile) {
	$result = db2_fetch_array($i5Connect,   "Select coalesce(HNXHND, ' ') as HNXHND, coalesce(HNROLE, ' ') as HNROLE, SYUSER.* From SYHAND inner join SYUSER on HNUSER=USUSER Where HNHAND='$eID' ");
	$row = db2_fetch_assoc($result);
	$activeRole    = trim($row['HNROLE']);
	$profileHandle = trim($row['HNXHND']);
	$profileName   = trim($row['USDESC']);
