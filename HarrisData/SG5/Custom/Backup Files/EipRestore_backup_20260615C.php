...
// 5 portal-level landing pages — FUID must be PCODE/REPORT for EIP nav to render the tab
foreach ($portals as $pcode => $pdesc) {
    ...
    runSql("SYURLM $pcode",
        "INSERT INTO S5HDSDATA.SYURLM
         ...
         SELECT '$fuid','$pdesc','$pdesc','_blank','$url','',' ','$pdescu',
...
// 5 top-level nav tabs (FPPAGE = '') — FPID must be PCODE/REPORT for EIP nav to render the tab
foreach ($portals as $pcode => $pdesc) {
    $fpid   = $pcode . '/REPORT';
    $pdescu = strtoupper($pdesc);
    runSql("SYPORT top $pcode",
        "INSERT INTO S5HDSDATA.SYPORT
         ...
         SELECT '$pcode',''  ,1,'$fpid','$pdesc','$pdesc',' ','$pdescu',
                CURRENT_TIMESTAMP,'RESTORE','RESTORE',' '
         FROM SYSIBM.SYSDUMMY1
         WHERE NOT EXISTS (
