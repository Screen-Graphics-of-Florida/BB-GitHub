...
// -- Qty Available ------------------------------------------------------------
//
//  Bill's specification, carried over UNCHANGED from the Kits Structure Report
//  (see KitsStructureReport.php, alias QTY_AVAILABLE):
//
//      Qty Available = OHQTY - (CMTMO + QOO + RESQ)
//
//          OHQTY  HDIWHS.IWOHQT   quantity on hand
//          QOO    HDIWHS.IWQOO    quantity on order
//          RESQ   HDIWHS.IWRESQ   quantity reserved
//          CMTMO  HDIPLT.IPCMTO   quantity committed to manufacturing
//
//  Do not substitute an availability formula from elsewhere in the codebase.
//  This one is Bill's, restated 2026-09-01 when the same question came up on the
//  Kits report, and both pages are deliberately held to it so the two screens
...
function bp_qtyAvail($conn, $items, &$sqlErr, &$timings) {
    ...
    $sql = "
        SELECT X.ITM AS ITM,
               SUM(X.OHQTY) - (SUM(X.CMTMO) + SUM(X.QOO) + SUM(X.RESQ)) AS QAVAIL
        FROM (
    ...
    foreach (bp_fetchAll($conn, $sql, 'qtyAvail', $sqlErr, $timings) as $r) {
        $out[trim((string)$r['ITM'])] = (float)$r['QAVAIL'];
    }
    return $out;
}
