...
$scriptName    = "ApplCashPaymentDeduction.php";
...
$dftOrderBy    = array(array("IVAINV","A","Invoice"),array("IVISEQ","A",""),array("PEENID","A",""));
$tabID         = "PAYMENT";
$programName   = "HARCED";
...
$NetBalSQL     = "Case When IECRTB='I' Then IVIVAM-IVNPOS-IVPPOS Else Coalesce((Select sum(-PEAMT) from ARPYEN Where (PEBCHN,PEBCHD,PEBCHB,PETYPE,PEID,trim(PECHK),PEPTYP,PEPMID,PEISEQ)=($fromBatchNumber,$fromBatchDate,$fromBatchBank,'$fromType',$fromID,'" . trim($fromDocument) . "',Coalesce(b.PEPTYP,' '),'$paymentID',a.IVISEQ)), 0) End ";
...
array("", "Credit", $viewCheckBoxURL, "3", "0"),
...
if ($tag == "ORDERBY"){
	...
	elseif ($sequence == "Invoice")        {$orby = array(array("IVAINV" ,"A","Invoice"),array("IVISEQ","A",""),array("IVISEQ","A",""),array("PEENID","A",""));}
	...
	elseif ($sequence == "EntryGenDed")    {$orby = array(array("PEGDED" ,"A","General Deduction"),array("IVAINV" ,"A","Invoice"),array("IVISEQ","A",""),array("PEENID","A",""));}
	elseif ($sequence == "EntryDedInv")    {$orby = array(array("PENINV" ,"A","Deduction Invoice"),array("IVAINV" ,"A","Invoice"),array("IVISEQ","A",""),array("PEENID","A",""));}
...
$stmtSQL .= "       ,Coalesce(PEGDED,' ') as PEGDED ";
$stmtSQL .= "       ,Case When Coalesce(PEGDED, ' ')='G' Then 'CHECKED' ELSE ' ' End as CHECKGENERAL ";
if ($CRIAUI=="A") {$stmtSQL .= " ,Coalesce(PENINV, Decimal($CRIINV,7,0))   as PENINV ";}
else              {$stmtSQL .= " ,Coalesce(PENINV, 0)   as PENINV ";}
...
$selectSQL .= " and IVIVCD<>'U' ";         // Cannot be Unapplied Cash
$selectSQL .= " and IVIVCD<>'S' ";         // Cannot Be Service Charge
$selectSQL .= " and IVIVAM>0)) ";          // Must Be Positive Invoice
...
if ($formatToPrint != "Y"){
	...
	print "\n <form class=\"formClass\" METHOD=POST NAME=\"Chg\" ACTION=\"{$baseURL}{$phpPath}{$scriptName}{$scriptVarBase}\" onSubmit=\"return false;\">";
	...
	if ($columnDisplay['PEGDED']=="Y" && ($HDMCRL==0 || $CRPRMC!="Y" || $BKCURT==$CFCURT) && ($CRBBAL!="Y" || $BMBCHT=="D")) {
		print "\n <td class=\"checkBoxCol\"><input type=\"checkbox\" name=\"addGenDed\" id=\"addGenDed\" value='G' title=\"General Deduction\"></td> ";
	}
	if ($columnDisplay['PENINV']=="Y") {
		if   ($CRGAUI=="E" || $CRIAUI=="E") {print "\n     <td class=\"entry\"><input type=\"text\"   name=\"addDedInvoice\" id=\"addDedInvoice\" size=\"7\" maxlength=\"7\"></td> ";}
		else                                {print "\n     <td class=\"entry\"><input type=\"hidden\" name=\"addDedInvoice\" id=\"addDedInvoice\"></td> ";}
	}
	if ($columnDisplay['PESBCD']=="Y")  {
	...
	if ($columnDisplay['PEPLT']=="Y") {
		...
		if ($HDPDRL>0) {print "\n                            <a href=\"{$homeURL}{$phpPath}PlantSearch.php{$genericVarBase}&amp;docName=Chg&amp;fldName=addPlant&amp;fldDesc=none\" onclick=\"$searchWinVar\">$searchImage</a>";}
		print "\n </td>  ";
	}
	...
	if ($columnDisplay['PESLSM']=="Y") {
		...
		print "\n                            <a href=\"{$homeURL}{$phpPath}SalesmanSearch.php{$genericVarBase}&amp;docName=Chg&amp;fldName=addSalesman&amp;fldDesc=none\" onclick=\"$searchWinVar\">$searchImage</a></td> ";
	}
	if ($columnDisplay['PETRMS']=="Y") {
		print "\n <td class=\"entry\" nowrap><input type=\"text\"     name=\"addTermsCode\" id=\"addTermsCode\" size=\"2\" maxlength=\"2\"> ";
		print "\n                            <a href=\"{$homeURL}{$phpPath}TermsCodeSearch.php{$genericVarBase}&amp;docName=Chg&amp;fldName=addTermsCode&amp;fldDesc=none\" onclick=\"$searchWinVar\">$searchImage</a></td> ";
	}
	// Add hidden fields needed for Javascript and the form
	if ($columnDisplay['PENINV']!="Y") {print "\n <td><input type=\"hidden\" name=\"addDedInvoice\" id=\"addDedInvoice\" value=\"\"></td> ";}
	...
	while ($row = db2_fetch_assoc($sqlResult, $startRow)){
		...
		if ($PESINV_ERROR!="") {$ivcdClass="inputnmbr"; $FldStyle="style=\"background-color: $errorBackground\" title=\"$PESINV_ERROR\"  ";}
		else                   {$FldStyle="";}
		...
		if ($row['PEAMT']!=0) {print "\n     <td class=\"inputnmbr\"><input type=\"text\" name=\"amt{$row['IVISEQ']}_{$row['PEENID']}\" id=\"amt{$row['IVISEQ']}_{$row['PEENID']}\" value=\"" . number_format($row['PEAMT'],2, '.', '') . "\" size=\"8\" maxlength=\"15\" $FldStyle onChange=\"editPEAMTDeduct('$row[IVISEQ]','$row[PEENID]','$row[MINPEENID]')\"></td> ";}
		else                  {print "\n     <td class=\"inputnmbr\"><input type=\"text\" name=\"amt{$row['IVISEQ']}_{$row['PEENID']}\" id=\"amt{$row['IVISEQ']}_{$row['PEENID']}\" value=\"\" size=\"8\" maxlength=\"15\" $FldStyle onChange=\"editPEAMTDeduct('$row[IVISEQ]','$row[PEENID]','$row[MINPEENID]')\"></td> ";}
		...
		if ($columnDisplay['IVBALN']=="Y") {
			...
			elseif ($row['PEENID']==$row['MINPEENID'])                    {print "\n <td class=\"$balClass\" $FldStyle>" . number_format($row[IVBALN],2) . "</td> ";}
		...
		if ($columnDisplay['PEGDED']=="Y" && ($HDMCRL==0 || $CRPRMC!="Y" || $BKCURT==$CFCURT) && ($CRBBAL!="Y" || $BMBCHT=="D")) {
			...
			if ($row['PEENID']==$row['MINPEENID'] && $row['IECRTB']=="A") {print "\n <input type=\"checkbox\" name=\"gded{$row['PEISEQ']}_{$row['PEENID']}\" id=\"gded{$row['PEISEQ']}_{$row['PEENID']}\" value='G' $row[CHECKGENERAL] onClick=\"editPEGDED('$row[PEISEQ]','$row[PEENID]','$row[MINPEENID]')\" title=\"General Deduction\"> ";}
			else                                                          {print "\n <input type=\"hidden\" name=\"gded{$row['IVISEQ']}_{$row['PEENID']}\" id=\"gded{$row['IVISEQ']}_{$row['PEENID']}\" value=\"" . rtrim($row['PEGDED']) . "\">&nbsp; ";}
			print "\n </td> ";
		}
		...
		if ($columnDisplay['PEORDT']=="Y") {
			...
			print "\n <td class=\"inputnmbr\"><input type=\"text\" name=\"ordt{$row['IVISEQ']}_{$row['PEENID']}\" id=\"ordt{$row['IVISEQ']}_{$row['PEENID']}\" value=\"" . rtrim($row['PEORDT']) . "\" size=\"6\" maxlength=\"6\" $FldStyle onChange=\"editPEORDT('$row[IVISEQ]','$row[PEENID]','$row[MINPEENID]')\"></td> ";
		}
		...
		if ($columnDisplay['PENINV']!="Y") {print "\n <td><input type=\"hidden\" name=\"ninv{$row['IVISEQ']}_{$row['PEENID']}\" id=\"ninv{$row['IVISEQ']}_{$row['PEENID']}\" value=\"" . rtrim($row['PENINV']) . "\"></td> ";}
