...
$page_title            = "Aged Accounts Receivable Report";
$scriptName            = "ARAgingReportDetail.php";
$scriptVarBase         = "{$genericVarBase}&amp;backHome=" . urlencode(trim($backHome)) . "&amp;reportSelType=" . urlencode(trim($reportSelType));
$baseURL               = "{$homeURL}{$phpPath}{$scriptName}{$scriptVarBase}";
$submitNoSelection     = "";
$submitCallProgram     = "CARAG2";
$submitEnvProgram      = "HARAG2";
$submitEnvPrinter      = "HARAG2PF";
...
if ($tag == "REPORT") {
	...
	print "\n   if (editdate(document.Chg.AgeAsOfDate) ";
	...
	if ($errFound != "" || $scheduleJobSwitch == "Y") {
		...
		if ($errFound != "") {
			...
			$Err_AGED=DecatErr_Field("@@aged", "AgeAsOfDate");
			$Err_IORD=DecatErr_Field("@@iord", "BaseAgingOn");
		...
		$AGED=Decat_Field("@@aged", $edtVar);
		$IORD=Decat_Field("@@iord", $edtVar);
	...
	print "\n     <table $contentTable> ";
	Build_Fld_Entry("Age As Of Date","AgeAsOfDate","inputnmbr","Date","",$AGED,$Err_AGED,"1","1","","","");
	Build_Fld_Entry("Base Aging On","BaseAgingOn","inputalph","AGEDATE","IORD",$IORD,$Err_IORD,"1","1","","","");
	print "\n     </table> ";
	print "\n </fieldset> ";
	...
	Build_Fld_Entry("","totalCollector","inputalph","YORN","TCLT",$TCLT,$Err_TCLT,"1","1","","","Y");
	Build_Fld_Entry("","pageCollector","inputalph","YORN","PCLT",$PCLT,$Err_PCLT,"1","1","","","Y");
	print "\n         </tr> ";
	DspErrMsg($Err_SCLT);
	...
	Build_Fld_Entry("","totalDate","inputalph","YORN","TDTE",$TDTE,$Err_TDTE,"1","1","","","Y");
	Build_Fld_Entry("","pageDate","inputalph","YORN","PDTE",$PDTE,$Err_PDTE,"1","1","","","Y");
	print "\n         </tr> ";
	DspErrMsg($Err_SDTE);
...
if ($tag == "WILDCARD" || $tag == "Edit_Data") {
	...
	Concat_Field("@@aged", $_POST['AgeAsOfDate']);
	Concat_Field("@@iord", strtoupper($_POST['BaseAgingOn']));
	...
	$returnValue=Selection_Edit_Handle("HARSAG_WD", $profileHandle, $dataBaseID, $submitSchedule, $errFound, $edtVar, $errVar, $wrnVar);
