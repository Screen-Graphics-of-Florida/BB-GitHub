...
%FUNCTION(dtw_directcall)  RtvColVal(IN    CHAR(32000) selectRecord,
			                            CHAR(1000)  fileName,
			                            CHAR(500)   fieldName,
			                            CHAR(1)     fieldMode,
                                      INOUT CHAR(32000) fieldVal)
  { %EXEC {HSYRFV_W.PGM %}
  %}

%{ Retrieve Column Description (With) %}
%FUNCTION(dtw_directcall)  RtvFldDescWith(IN    CHAR(5000)  withRecord,
...
%FUNCTION(dtw_directcall) checkFilePrefix (INOUT CHAR(4) filePrefix,
			                                 CHAR(1) prefixExists)
  {
      %EXEC {HHDCKF_W.PGM %}
  %}
...
%FUNCTION(dtw_directcall) Env_Overrides (IN    CHAR(10) profileHandle,
                                               CHAR(2)  apid,
                                               CHAR(10) envProgram,
                                               CHAR(10) envPrinter,
...
%{ Payer User View %}
%FUNCTION(DTW_directcall)  PayerUserView(INOUT CHAR(64)  profileHandle,
                                               CHAR(1)   userPass,
	                                              DEC(7,0)  payer,
	                                        IN    CHAR(1)   testOvrFields)
  { %EXEC {HARPYRH_W.PGM %}
...
%FUNCTION(DTW_directcall)  WFWrkItmUserView(INOUT CHAR(64)    profileHandle,
                                                  CHAR(2)     dataBaseID,
                                                  CHAR(1)     userPass,
                                                  CHAR(32000) edtVar)
{ %EXEC {HWFUSV_W.PGM %}
...
%FUNCTION(dtw_directcall) Get_Qty_ItemPlt (INOUT  CHAR(32000) edtVar)
{%EXEC {HHDSWQ_W.PGM %}
%}
...
%{ Retrieve Order Type User Defined Descriptions %}
...
%{ Load A/P Aging %}
%FUNCTION(dtw_directcall) LoadAPAging(INOUT CHAR(64)  profileHandle)
  {
      %EXEC {HAPAGI_W.PGM %}
  %}
...
%FUNCTION(dtw_directcall) RetWhsNbr(IN    CHAR(64) profileHandle,
                                          CHAR(2)  dataBaseID,
                                          CHAR(10) subroutine,
                                          DEC(3,0) plantNumber,
                                          CHAR(15) itemNumber,
                                    INOUT CHAR(3)  returnWarehouse)
  {
      %EXEC {SHDRWS_W.PGM %}
  %}
                             ...
                             INOUT CHAR(30) dftPltName)
{
  ...
  %if (dftPltNumber<="0")
      @dtw_assign(dftPltNumber, V_HDDPLT)
  %endif
...
%{ Convert Data %}
%FUNCTION(dtw_directcall) Convert_Data (INOUT CHAR(10) dataType,
                                              CHAR(1)  mode,
                                              CHAR(30) value)
{%EXEC {HSYDCV_W.PGM %}
%}

%{ Retrieve Default Stock Location for Plant Item %}
%FUNCTION(dtw_directcall) RetDefStkLoc(IN    CHAR(64) profileHandle,
                                             CHAR(2)  dataBaseID,
                                             CHAR(15) subprocedure,
                                             DEC(3,0) plantNumber,
                                             CHAR(15) itemNumber,
                                       INOUT CHAR(3)  returnWhs,
                                             CHAR(1)  returnSla,
                                             CHAR(3)  returnStkr,
                                             CHAR(4)  returnAile,
                                             CHAR(8)  returnSloc,
...
%{ Check Credit Card Interface %}
%FUNCTION(dtw_directcall) CrCardProc  (INOUT CHAR(2)  apid,
                                             CHAR(10) ws,
                                             CHAR(1)  dataQ,
                                             CHAR(10) ccPgm,
                                             CHAR(7)  ccMsg)
...
%FUNCTION(dtw_directcall) Calc_Date_Offset(INOUT CHAR(10) dateISO,
                                                 CHAR(1)  addSub,
                                                 CHAR(5)  duration,
						                         CHAR(1) ymd)
  {%EXEC {HSYCDT_W.PGM%}
